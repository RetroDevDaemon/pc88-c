...
void LZ77Enc::Decode(uint8* dest, const uint8* src, const uint8* test)
{
	...
	while (1)
	{
		...
		{
#ifdef DBGLOG
			fprintf(fp, "%.4x: (0x%.2x)¥n", dest-dorg, *src);
#endif
			*dest++ = *src++;
			if (dest[-1] != *test++)
			{
				printf("decode error: 1[xx]¥n");
				return;
			}
		}
		else
...
#ifdef DBGLOG
			fprintf(fp, "%.4x: %d, %d¥n", dest - dorg, -x, len);
#endif
			int l0 = len;
			for (; len>0; len--)
			{
				...
				if (dest[0] != *test++)
				{
					printf("decode error: %d, %d (%.2x)¥n", x, l0, k);
					return;
				}
				dest++;
			}
